...
bin/console doctrine:database:drop --force --env=test --no-debug --if-exists
...
bin/console event-store:projection:run post_projection --env=test &
bin/phpunit --stop-on-failure
bin/console event-store:projection:stop post_projection --env=test &
